...
void CJoker::Late_Tick(_float fTimeDelta)
{
...
HRESULT CJoker::Add_Components()
{
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Joker_Anim"),
...
CJoker* CJoker::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		...
		MSG_BOX("Failed to Created : CJoker");
	}
...
CGameObject* CJoker::Clone(void* pArg)
{
	...
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CJoker");
	}
