...
HRESULT CCube::Add_Components(void* pArg)
{
	/* Com_Renderer */
	...
	MODELDESC* a = static_cast<MODELDESC*>(pArg);
	/* Com_VIBuffer */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, a->strPath,
...
HRESULT CCube::Bind_ShaderResources()
{
	...
	//XMStoreFloat4x4(&shadowViewMat, pGameInstance->Get_Transform_Matrix(CPipeLine::D3DTS_LIGHTWORLD));
	//shadowViewMat.m[3][1] += 500;
	//XMStoreFloat4x4(&shadowViewMat,XMMatrixInverse(nullptr, XMLoadFloat4x4(&shadowViewMat)));
