...
HRESULT CSnow_Red::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, { 0,0,10 });
...
void CSnow_Red::Tick(_float fTimeDelta)
{
	...
	if (Get_PointInstanceDesc()->fLifeTime > Get_PointInstanceDesc()->fLifeMaxTime - Get_PointInstanceDesc()->fDisappearTime)
			m_Color.w -= 1/Get_PointInstanceDesc()->fDisappearTime * fTimeDelta;
}
...
HRESULT CSnow_Red::Bind_ShaderResources()
{
	...
	if (FAILED(m_pShaderCom->Bind_RawValue("g_Color", &m_Color, sizeof(_float4))))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Bind_RawValue("g_Size", &m_Size, sizeof(_float2))))
		return E_FAIL;

	//_float4x4 m;
	//XMStoreFloat4x4(&m, XMMatrixRotationQuaternion(XMQuaternionRotationAxis({ 0,0,1 }, XMConvertToRadians(45.f))));
	//if (FAILED(m_pShaderCom->Bind_Matrix("g_RotationMatrix", &m)))
	//	return E_FAIL;
