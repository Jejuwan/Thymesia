...
HRESULT CTarget_Manager::Add_RenderTarget(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const wstring & strTargetTag, _uint iSizeX, _uint iSizeY, DXGI_FORMAT ePixelFormat, const _float4 & vClearColor,CRenderTarget::TYPE eType)
{
	if (nullptr != Find_RenderTarget(strTargetTag))
		return E_FAIL;

	CRenderTarget*		pRenderTarget = CRenderTarget::Create(pDevice, pContext, iSizeX, iSizeY, ePixelFormat, vClearColor, eType);
...
HRESULT CTarget_Manager::Begin_MRT(ID3D11DeviceContext * pContext, const wstring & strMRTTag)
{
	...
	ID3D11RenderTargetView*		pRTV[8] = { nullptr };
	...
	{
		if(pRenderTarget->Get_Type() == CRenderTarget::TYPE::RENDER_TARGET)
		{
			pRenderTarget->Clear();
			pRTV[iNumRTVs++] = pRenderTarget->Get_RTV();
		}
		else if (pRenderTarget->Get_Type() == CRenderTarget::TYPE::DEPTH_STENCIL)
		{
			pRenderTarget->ClearDSV();
			pDSV = pRenderTarget->Get_DSV();
			pContext->OMSetRenderTargets(0, nullptr, pDSV);
		}
	}
	
	pContext->OMSetRenderTargets(iNumRTVs, pRTV, m_pDepthStencilView);
